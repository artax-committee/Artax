...
class ArtaxManyValueRefundsFromSameTxTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        tx = CTransaction()
        tx.vin = [make_vin(self.node, int(2*(COIN + XAX_MIN_GAS_PRICE*100000)))]
        tx.vout = []
        tx.vout.append(CTxOut(int(COIN), CScript([b"\x04", CScriptNum(100000), CScriptNum(XAX_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL])))
        tx.vout.append(CTxOut(int(COIN), CScript([b"\x04", CScriptNum(100000), CScriptNum(XAX_MIN_GAS_PRICE), hex_str_to_bytes("00"), hex_str_to_bytes(contract_address), OP_CALL])))
...
if __name__ == '__main__':
    ArtaxManyValueRefundsFromSameTxTest().main()
