...
def convert_btc_address_to_xax(addr, main=False):
...
def convert_btc_bech32_address_to_xax(addr, main=False):
...
class DGPState:
    ...
    def send_set_initial_admin(self, sender):
        ...
        self.node.sendtocontract(self.contract_address, self.abiSetInitialAdmin, 0, 2000000, XAX_MIN_GAS_PRICE_STR, sender)

    def send_add_address_proposal(self, proposal_address, type1, sender):
        self.node.sendtoaddress(sender, 1)
        self.node.sendtocontract(self.contract_address, self.abiAddAddressProposal + proposal_address.zfill(64) + hex(type1)[2:].zfill(64), 0, 2000000, XAX_MIN_GAS_PRICE_STR, sender)

    def send_remove_address_proposal(self, proposal_address, type1, sender):
        self.node.sendtoaddress(sender, 1)
        self.node.sendtocontract(self.contract_address, self.abiRemoveAddressProposal + proposal_address.zfill(64) + hex(type1)[2:].zfill(64), 0, 2000000, XAX_MIN_GAS_PRICE_STR, sender)

    def send_change_value_proposal(self, uint_proposal, type1, sender):
        self.node.sendtoaddress(sender, 1)
        self.node.sendtocontract(self.contract_address, self.abiChangeValueProposal + hex(uint_proposal)[2:].zfill(64) + hex(type1)[2:].zfill(64), 0, 2000000, XAX_MIN_GAS_PRICE_STR, sender)
