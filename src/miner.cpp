...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setArtaxSchedule(xaxDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = xaxDGP.getBlockSize(nHeight);
    minGasPrice = xaxDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = xaxDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    ArtaxTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractArtaxTX resultConverter;
    if(!convert.extractionArtaxTransactions(resultConverter)){
    ...
    for(ArtaxTransaction xaxTransaction : xaxTransactions){
        txGas += xaxTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + xaxTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(xaxTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, xaxTransactions, hardBlockGasLimit);
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "xaxstake";
...
void StakeArtaxs(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
