...
#include <xax/xaxstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

ArtaxState::ArtaxState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = ArtaxState::openDB(_path + "/xaxDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ArtaxState::ArtaxState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute ArtaxState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, ArtaxTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createArtaxAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            xax::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> ArtaxState::vins() const // temp
{
...
void ArtaxState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* ArtaxState::vin(dev::Address const& _a) const
{
    return const_cast<ArtaxState*>(this)->vin(_a);
}

Vin* ArtaxState::vin(dev::Address const& _addr)
{
...
// void ArtaxState::commit(CommitBehaviour _commitBehaviour)
// {
...
void ArtaxState::kill(dev::Address _addr)
{
...
void ArtaxState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address ArtaxState::createArtaxAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void ArtaxState::deleteAccounts(std::set<dev::Address>& addrs){
...
void ArtaxState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void ArtaxState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
