#include <xax/xaxDGP.h>

void ArtaxDGP::initDataEIP158(){
...
bool ArtaxDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule ArtaxDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t ArtaxDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t ArtaxDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t ArtaxDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t ArtaxDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool ArtaxDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void ArtaxDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void ArtaxDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void ArtaxDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void ArtaxDGP::createParamsInstance(){
...
dev::Address ArtaxDGP::getAddressForBlock(unsigned int blockHeight){
...
void ArtaxDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ArtaxDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void ArtaxDGP::parseStorageOneUint64(uint64_t& value){
...
void ArtaxDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule ArtaxDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
