...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // xax
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // xax
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // xax
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // xax
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CTxDestination xaxSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(xaxSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&xaxSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of xax addresses\n"
