...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-aggressive-scratching", "Check more often to publish immediately when valid block is found.", false, OptionsCategory::BLOCK_CREATION);
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path xaxStateDir = GetDataDir() / "stateArtax";
                bool fStatus = fs::exists(xaxStateDir);
                const std::string dirArtax(xaxStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsArtaxstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<ArtaxState>(new ArtaxState(dev::u256(0), ArtaxState::openDB(dirArtax, hashDB, dev::WithExisting::Trust), dirArtax, existsArtaxstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::xaxMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(xaxStateDir.string()));
                if (fReset) {
