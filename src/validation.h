...
#include <xax/storageresults.h>


extern std::unique_ptr<ArtaxState> globalState;
...
using ExtractArtaxTX = std::pair<std::vector<ArtaxTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // xax
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashArtaxRoot,
    const dev::h256& oldHashStateRoot, const std::vector<ArtaxTransaction>& transactions);
...
class ArtaxTxConverter{

public:

    ArtaxTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionArtaxTransactions(ExtractArtaxTX& xaxTx);
...
private:
    ...
    ArtaxTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<ArtaxTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
