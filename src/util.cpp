...
const char * const BITCOIN_CONF_FILENAME = "xax.conf";
const char * const BITCOIN_PID_FILENAME = "xaxd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Artax
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Artax
    // Mac: ~/Library/Application Support/Artax
    // Unix: ~/.xax
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Artax";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Artax";
#else
    // Unix
    return pathRet / ".xax";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Artax Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Artax Core developers";
    }
