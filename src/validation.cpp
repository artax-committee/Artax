...
std::unique_ptr<ArtaxState> globalState;
...
const std::string strMessageMagic = "Artax Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = xaxDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = xaxDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            ArtaxTxConverter converter(tx, NULL);
            ExtractArtaxTX resultConverter;
            if(!converter.extractionArtaxTransactions(resultConverter)){
            ...
            std::vector<ArtaxTransaction> xaxTransactions = resultConverter.first;
            std::vector<EthTransactionParams> xaxETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(ArtaxTransaction xaxTransaction : xaxTransactions){
                sumGas += xaxTransaction.gas() * xaxTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, xaxTransaction.gasPrice());
                } else {
                    txMinGasPrice = xaxTransaction.gasPrice();
                }
                VersionVM v = xaxTransaction.getVersion();
                if(v.format!=0)
                ...
                if(xaxTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(xaxTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(xaxTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)xaxTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(xaxETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > xaxTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s XAX additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // xax
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // xax
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    ArtaxTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<ArtaxTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path xaxDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(xaxDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(xaxDir.string(), std::ios::in | std::ios::out);
...
bool ArtaxTxConverter::extractionArtaxTransactions(ExtractArtaxTX& xaxtx){
    std::vector<ArtaxTransaction> resultTX;
    ...
    xaxtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool ArtaxTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool ArtaxTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
ArtaxTransaction ArtaxTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    ArtaxTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = ArtaxTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = ArtaxTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // xax
    ArtaxDGP xaxDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setArtaxSchedule(xaxDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = xaxDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = xaxDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = xaxDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // xax
...
///////////////////////////////////////////////////////////////////////////////////////// xax
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            ArtaxTxConverter convert(tx, &view, &block.vtx);

            ExtractArtaxTX resultConvertArtaxTX;
            if(!convert.extractionArtaxTransactions(resultConvertArtaxTX)){
            ...
            if(!CheckMinGasPrice(resultConvertArtaxTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertArtaxTX.first, blockGasLimit);
            ...
            for(ArtaxTransaction& qtx : resultConvertArtaxTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertArtaxTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertArtaxTX.first[k].from(), resultConvertArtaxTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // xax
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // xax
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // xax
            globalState->setRootUTXO(oldHashUTXORoot); // xax
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // xax
        uint32_t sizeBlockDGP = xaxDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Artax PoS we need to sync this after every block to ensure txdb is populated for
