...
inline void initState(){
    ...
    const std::string dirArtax = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<ArtaxState>(new ArtaxState(dev::u256(0), ArtaxState::openDB(dirArtax, hashDB, dev::WithExisting::Trust), dirArtax + "/xaxDB", dev::eth::BaseState::Empty));
...
inline dev::Address createArtaxAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline ArtaxTransaction createArtaxTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    ArtaxTransaction txEth;
    if(recipient == dev::Address()){
        txEth = ArtaxTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = ArtaxTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<ArtaxTransaction> txs){
