...
void checkResult(bool isCreation, std::vector<ArtaxTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    ArtaxTxConverter converter(transaction, NULL);
    ExtractArtaxTX xaxTx;
    BOOST_CHECK(converter.extractionArtaxTransactions(xaxTx));
    std::vector<ArtaxTransaction> result = xaxTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(xaxtxconverter_tests, TestingSetup)
