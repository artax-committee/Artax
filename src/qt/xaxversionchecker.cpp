...
#define paternVersion "xax-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

ArtaxVersionChecker::ArtaxVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

ArtaxVersionChecker::~ArtaxVersionChecker()
{

}

bool ArtaxVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> ArtaxVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(XAX_RELEASES)));
    QEventLoop event;
...
Version ArtaxVersionChecker::getMaxReleaseVersion()
{
