...
bool ToHash160(const std::string& strArtaxAddress, std::string& strHash160)
{
    CTxDestination xaxAddress = DecodeDestination(strArtaxAddress);
    if(!IsValidDestination(xaxAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&xaxAddress);
    if(keyid){
...
bool ToArtaxAddress(const std::string& strHash160, std::string& strArtaxAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination xaxAddress = keyid;
    if(IsValidDestination(xaxAddress)){
        strArtaxAddress = EncodeDestination(xaxAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToArtaxAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToArtaxAddress(tokenEvent.receiver, tokenEvent.receiver);
