...
*Setup instructions for a Gitian build of Artax Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Artax
...
- [Building Artax Core](#building-xax-core)
...
Clone the git repositories for xax and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/xaxproject/xax --recursive
git clone https://github.com/xaxproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/xax/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the xax repository under 'Fetch and create inputs' to install sources which require
...
To build Artax Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the xax repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xax/.git/
...
For example:
```bash
URL=https://github.com/laanwj/xax.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit xax=${COMMIT} --url xax=${URL} ../xax/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xax=${COMMIT} --url xax=${URL} ../xax/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xax=${COMMIT} --url xax=${URL} ../xax/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-xax:
./bin/gbuild --commit xax=${COMMIT},cpp-eth-xax=${ETHCOMMIT} --url xax=${URL},cpp-eth-xax=${ETHURL} ../xax/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the xax git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../xax/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/xaxproject/xax-detached-sigs.git

BTCPATH=/some/root/path/xax
SIGPATH=/some/root/path/xax-detached-sigs

./bin/gbuild --url xax=${BTCPATH},signature=${SIGPATH} ../xax/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xax-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xax-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/xax-osx-build.assert
```
...
[xaxproject/gitian.sigs](https://github.com/xaxproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@xax.org) and he will commit them.