...
/*
 * title:   ArtaxCP
 * desc:    Import Artax's blk.dat files (the blockchain) in to a Neo4j graph database.
 * author:  Greg Walker
 * license: GPLv3
 * website: learnbitcoin.com
*/
...
if (!file_exists(BLOCKS)) {
    exit("Couldn't find ".BLOCKS.PHP_EOL."Make sure you have entered the correct path to Artax's blk*.dat files.\n");
}
...
$start = $redis->hGet('ArtaxCP', 'blk.dat') ?: 0; // which blk.dat file to start with
$startfp = $redis->hGet('ArtaxCP', 'fp') ?: 0; // Zero if not set
...
while(true) { // Keep trying to read files forever
    ...
    $redis->hSet('ArtaxCP', 'blk.dat', $start);
    ...
    while(true) { // Read through a blk*.dat file
        ...
        while (true) {
            ...
            // Magic Bytes
            $magicbytes = TESTNET ? '20ee32bc' : '54dc12ae';
        ...
        // If we don't have a height, save this block hash for future updating
            ...
            $redis->hset("ArtaxCP:orphans", $prevblock, $blockhash);

            // print out how many orphan blocks we have saved in Redis
            echo '  - blocks needed = '.$redis->hLen('ArtaxCP:orphans')."\n";
        }
        ...
        if ($redis->hExists('ArtaxCP:orphans', $blockhash) && $height !== NULL) {
            ...
            foreach ($chainabove as $orphan) {
                ...
                $redis->hdel("ArtaxCP:orphans", $orphan);

            }

            // Remove this current block from redis orphan too
            $redis->hdel("ArtaxCP:orphans", $blockhash);
        ...
        if ($height > $redis->hget("ArtaxCP:tip", 'height')) {
            $redis->hset("ArtaxCP:tip", 'height', $height);
            $redis->hset("ArtaxCP:tip", 'hash', $hash);
        }
    ...
    $b--;
    $redis->hSet('ArtaxCP:log', $file, "[$b] $dat_time mins");
